    ...
    public class GUIControls : MonoBehaviour
    {
        ...
        private void Sprinting()
        {
			if (rpgCharacterController.hasNoWeapon) {
				...
				if (useSprint && rpgCharacterController.CanStartAction("Sprint")) { rpgCharacterController.StartAction("Sprint"); } 
				else if (!useSprint && rpgCharacterController.CanEndAction("Sprint")) { rpgCharacterController.EndAction("Sprint"); }
			}
        }
        ...
        private void Crouching()
        {
            ...
            if (useCrouch && rpgCharacterController.CanStartAction("Crouch")) {  rpgCharacterController.StartAction("Crouch"); } 
			else if (!useCrouch && rpgCharacterController.CanEndAction("Crouch")) { rpgCharacterController.EndAction("Crouch"); }
		...
		private void WeaponSwitching()
		{
			...
			if (!rpgCharacterController.isRelaxed) {
				if (GUI.Button(new Rect(1115, 240, 100, 30), "Relax")) {
					if (useInstant) { rpgCharacterController.StartAction("Relax", true); } else { rpgCharacterController.StartAction("Relax"); }
				}
			}
			...
			if (hipShooting) { rpgCharacterController.StartAction("HipShoot"); } else { rpgCharacterController.EndAction("HipShoot"); }
