    ...
    public class RPGCharacterMovementController : SuperStateMachine
    {
        ...
        */
        ...
        protected override void LateGlobalSuperUpdate()
        {
			...
			// Aiming.
			if (rpgCharacterController.isAiming || rpgCharacterController.isStrafing) { Strafing(rpgCharacterController.aimInput); } 

			// Facing.
			else if (rpgCharacterController.isFacing) { Facing(rpgCharacterController.faceInput); } 
        ...
        private void Jump_EnterState()
        {
            ...
            if ((RPGCharacterState)lastState == RPGCharacterState.Swim) { currentVelocity = new Vector3(currentVelocity.x, strokeSpeed, currentVelocity.z); } 
		...
		private void Crawl_SuperUpdate()
		{
			...
			if (rpgCharacterController.canMove) {
				...
				currentVelocity = Vector3.MoveTowards(currentVelocity, cameraRelativeInput * moveSpeed, moveAccel * superCharacterController.deltaTime);
			}
        ...
        private void Swim_SuperUpdate()
        {
            ...
            if (currentVelocity.magnitude > Mathf.Max(swimSpeed, strokeSpeed)) {
                currentVelocity = Vector3.MoveTowards(currentVelocity, Vector3.zero, waterFriction * waterFriction * superCharacterController.deltaTime);
            }
            // Horizontal swim movement.
            if (cameraRelativeInput.magnitude > 0) {
                currentVelocity = Vector3.MoveTowards(currentVelocity, cameraRelativeInput * swimSpeed, swimAccel * superCharacterController.deltaTime);
            } else {
                // Apply friction to slow character to a halt on horizontal axes.
                currentVelocity = Vector3.MoveTowards(currentVelocity, new Vector3(0, currentVelocity.y, 0), waterFriction * superCharacterController.deltaTime);
            }
            // Apply friction to slow character to a halt on vertical axis.
            currentVelocity = Vector3.MoveTowards(currentVelocity, new Vector3(currentVelocity.x, 0, currentVelocity.z), waterFriction * superCharacterController.deltaTime);
		...
		private void RotateTowardsDirection(Vector3 direction)
		{
			...
			transform.rotation = Quaternion.RotateTowards(lookRotation, transform.rotation, Time.deltaTime * rotationSpeed);
		}
		...
		private void Facing(Vector3 direction)
		{
